...
class ShareService {
     ...
     */
    static async calculateDilution(targetOwnership, targetShares) {
        ...
        const totalCurrentShares = shareholders.reduce((sum, s) => sum + s.totalShares, 0);
        const totalNewShares = totalCurrentShares + targetShares;
        // Calculate new ownership percentages
        const allocations = shareholders.map((shareholder) => {
     ...
     */
    static async calculateSingleTransfer(fromShareholderId, targetShares) {
        // Get the selling shareholder
        const fromShareholder = await prismaClient_1.prisma.shareholder.findUnique({
     ...
     */
    static async executeDilution(newShareholderData, allocations) {
     ...
     */
    static async executeSingleTransfer(newShareholderData, fromShareholderId, transferShares) {
        return await prismaClient_1.prisma.$transaction(async (tx) => {
     ...
     */
    static async executeShareTransfer(fromShareholderId, toShareholderId, transferShares, shareClassId, price) {
        return await prismaClient_1.prisma.$transaction(async (tx) => {
    ...
    static async getTotalShares() {
        const result = await prismaClient_1.prisma.shareholder.aggregate({
            _sum: {
                totalShares: true
            },
            where: {
